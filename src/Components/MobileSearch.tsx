...
const MobileSearch: FC<any> = ({ address, location, addItem, counts, removeItem, setPropertyCoordinates, setIsSelectedFromDropdown, setDropdownLocationError, getCurrentLocation, isOpen, calculatePrediction, outsideRef }) => {
    let labelStyle = {
        fontSize: '14px',
        fontFamily: 'GTBold'
    }
    ...
    return (
        <Box w={'100%'}>
            ...
            <Flex justifyContent={'space-between'} alignItems={'center'} w={'100%'} maxW={'650px'} borderRadius={'12px'} py={'11px'} px={'15px'} border={'1px solid  #e2e8f0'} boxShadow={'0px 1px 3px -2px rgba(158, 157, 164, 0.49)'}>
                <Flex pos={'relative'} flex={1}>
                    ...
                    <Flex>
                        ...
                        <Box onClick={getCurrentLocation} cursor={'pointer'} alignSelf={'center'} bg={'rgb(255, 174, 242)'} borderRadius="8px" p={'7px'} >
            ...
            <Flex px={'35px'} mt={3} justifyContent={'space-between'} alignItems={'center'} w={'100%'} maxW={'650px'} borderRadius={'12px'} py={'11px'} border={'1px solid  #e2e8f0'} boxShadow={'0px 1px 3px -2px rgba(158, 157, 164, 0.49)'}>
                ...
                <Flex flexDirection={'column'} alignItems={'center'} flex={1} borderLeft="1px solid #e2e8f0"  >
                ...
                <Flex alignItems={'flex-end'} flexDirection={'column'} flex={1} borderLeft="1px solid #e2e8f0"  >
                    <Text sx={labelStyle}>Guests</Text>
                    <Flex alignItems={'center'} color={'rgb(158, 157, 164)'}  >
                        <AiOutlinePlus onClick={() => addItem('Guests')} cursor={'pointer'} />
                        <Text mx={2} fontSize="14px" >{guests}</Text>
                        <AiOutlineMinus onClick={() => removeItem('Guests')} cursor={'pointer'} />
                    </Flex>
                </Flex>
            </Flex >
            <Button onClick={calculatePrediction} mt={6} w="100%" height={'36px'} fontSize={'15px'} bg={'rgb(247, 34, 219)'} color={'white'} fontFamily={'GTBold'}>
                Calculate
            </Button>
